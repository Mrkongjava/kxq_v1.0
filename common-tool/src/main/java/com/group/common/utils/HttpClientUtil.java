...
public class HttpClientUtil {
    ...
    public static String post(String url, NameValuePair[] param) throws Exception {
        String result = "";
        ...
        headers.add(new Header("Content-Type", "application/application/x-www-form-urlencoded; charset=UTF-8"));
        headers.add(new Header("Accept", "text/xml,text/javascript,text/html"));
        client.getHostConfiguration().getParams().setParameter("http.default-headers", headers);
        // 设置请求头——end

        post.setRequestBody(param);// 设置请求参数
        client.executeMethod(post);// 执行post请求

        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(post.getResponseBodyAsStream()));// 接收响应结果字符流
        StringBuffer queryResult = new StringBuffer();// 响应结果（返回json）
        String str = "";
        while ((str = bufferedReader.readLine()) != null) {// 读取流
            queryResult.append(str);// 拼接响应结果
        }
